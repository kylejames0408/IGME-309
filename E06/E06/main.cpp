...
void init(void)
{
	...
	//g_mesh.computeAABB(); // cannot possibly call in main.cpp since loadFromObj() calls prepareBufferObjects() which removes vertex data - computeAABB() called in loadFromObj() line 263-264
}
...
void display()
{
	...
	g_mesh.draw();
	g_mesh.drawAABB(); // draw the bounding box wireframe
